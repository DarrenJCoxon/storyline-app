...
//   1. Derived from .novel-writer/state.json (_meta, genre, premise)
...
export async function assemble(context) {
  ...
  if (!(await pathExists(manuscriptDir))) {
    throw new Error(
      ...
      `Run \`nw init\` to create it, or edit .novel-writer/state.json ` +
...
async function readState(projectPath) {
  ...
  if (!(await pathExists(statePath))) {
    throw new Error('Could not find .novel-writer/state.json — is this a novel project?');
  }
  try {
    const raw = await readFile(statePath, 'utf-8');
    return JSON.parse(raw);
  } catch (err) {
    throw new Error(`Could not parse .novel-writer/state.json: ${err.message}`);
  }
}
