...
export async function applyTheme(context) {
  ...
  const themeId = await resolveThemeId(context);
...
async function resolveThemeId(context) {
  const configPath = resolve(context.projectPath, 'compile.config.json');
  if (!(await pathExists(configPath))) return DEFAULT_THEME_ID;
  try {
    const config = JSON.parse(await readFile(configPath, 'utf-8'));
    const id = typeof config?.theme === 'string' ? config.theme.trim() : '';
...
async function loadTheme(themeId) {
  ...
  const cssPath = resolve(themeDir, 'theme.css');
  ...
  const [css, metaRaw] = await Promise.all([
