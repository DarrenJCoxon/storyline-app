...
export async function applyTheme(context) {
  ...
  const { themeId, paragraphStyle, themeOverrides } = await resolveThemeConfig(context);
  ...
  const effectiveCss = theme.css + overrideCss + paragraphStyleOverride(paragraphStyle);
...
async function resolveThemeConfig(context) {
  ...
  try {
    ...
    return { themeId, paragraphStyle, themeOverrides };
  } catch {
    return defaults;
  }
}
