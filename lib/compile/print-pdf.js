// Print PDF packager — produces a press-ready 6x9 PDF.
//
// Pipeline:
//   1. Assemble a self-contained HTML document (theme CSS + paged.js
//      polyfill + all book sections). Written to disk as
//      <slug>-print-preview.html so writers can inspect pagination.
...
//      @page size (6in × 9in) from the theme CSS.
//   6. Save to <slug>-print-6x9.pdf.
...
export async function packagePrintPdf(context) {
  ...
  const pdfPath = resolve(outputDir, `${slug}-print-6x9.pdf`);
...
async function renderPdf({ previewHtmlPath, pdfPath }) {
  ...
  try {
    ...
    await page.pdf({
      path: pdfPath,
      preferCSSPageSize: true,   // use @page size from theme CSS (6in × 9in)
...
async function buildPagedHtml({ metadata, theme }) {
  ...
  // Inject string-set for the running header via an inline style. This
  // is more reliable than a hidden marker element — no phantom DOM node
  // to affect pagination, and it doesn't matter where the title ends
  // up on the first page.
  const titleForCss = cssQuote(metadata.title);
...
${theme.css}
  ...
  <style>
    /* Compile-time string-set: book title captured for the verso header */
    body { string-set: book-title "${titleForCss}"; }
  </style>
</head>
<body>
${frontHtml}
...
// Escape a string for use inside a CSS `string-set` double-quoted value.
// Mainly guards against writers whose book titles contain literal quotes.
function cssQuote(s) {
  if (s == null) return '';
