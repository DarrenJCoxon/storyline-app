...
// The /novel skill calls `nw memory sync` to get entries not yet pushed
// to mcp__odd-flow__memory_store, and `nw memory mark-synced <ids...>`
...
const LOG_FILE = '.novel-writer/memory.jsonl';
const SYNCED_FILE = '.novel-writer/memory.synced';

async function readSyncedIds() {
  const path = resolve(process.cwd(), SYNCED_FILE);
...
async function readAllEntries() {
  const path = resolve(process.cwd(), LOG_FILE);
...
export async function getPendingEntries() {
  const [all, synced] = await Promise.all([readAllEntries(), readSyncedIds()]);
  const pending = [];
  const seen = new Set();
  for (const e of all) {
...
export async function markSynced(ids) {
  ...
  await ensureDir(resolve(process.cwd(), '.novel-writer'));
  const current = await readSyncedIds();
...
export async function getSyncStatus() {
  ...
  return {
    ...
    logPath: resolve(process.cwd(), LOG_FILE),
    syncedPath: resolve(process.cwd(), SYNCED_FILE),
  };
}
