...
const builders = {
  ...
  premise(state, ns, tags) {
    ...
    if (p.seriesContext?.isSeries) {
      out.push(entry(ns, 'premise:series', `${p.seriesContext.seriesTitle} (book ${p.seriesContext.currentBookNumber || 1} of ${p.seriesContext.bookCount || '?'})`, [...tags, 'series']));
      out.push(entry(ns, 'premise:series-arc', p.seriesContext.overallArc, [...tags, 'series']));
      out.push(entry(ns, 'premise:first-book-focus', p.seriesContext.firstBookFocus, [...tags, 'series']));
    }
    return out;
  },
  ...
  bStory(state, ns, tags) {
    ...
    return [
      ...
      entry(ns, 'bstory:theme', b.themeConnection, tags),
    ];
  },
