...
export async function buildCritiqueBrief(chapterRef, state, projectPath = process.cwd()) {
  ...
  if (!chapterPlan) {
    ...
    // skips the corresponding `storyline-cli save chapterOutline`
      ...
      return {
        error: {
          ...
          message:
            ...
            `prose doc without invoking \`storyline-cli save chapterOutline\`. ` +
            `Run \`storyline-cli doctor\` to confirm the drift across all stages, ` +
