...
//
// `nw manuscript compare` produces a structured diff report surfacing
  ...
  for (const draft of snapshot.chapters) {
    ...
    if (!plan) {
      findings.push({
        ...
        fix: `Either add it to the plan (nw save chapterOutline) or remove/merge the draft chapter.`,
      });
      continue;
    }
    ...
    if (plan.plannedScenes > 0 && draft.sceneCount !== plan.plannedScenes) {
      findings.push({
        ...
        fix: `Either update the plan's scene breakdown (nw save chapterOutline) or adjust the draft's scene structure.`,
      });
    }
