> **Extension adaptation:** the `/follow-up` skill runs via the `Storyline: View Manuscript Notes` command in the extension, not as a slash command. The `{{TBD}}` markers are surfaced in the Notes panel.
