> **Extension adaptation:** in the VS Code extension, the startup protocol runs inside the chat panel. CLI commands (`npx storyline-vsc`) are handled internally — never ask the writer to run them.
