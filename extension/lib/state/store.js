...
function findLatestProject() {
  ...
  const statePath = resolve(checkPath, '.storyline', 'state.json');
...
function mergeWithDefaults(state) {
  return {
    ...
    // mode/pipeline/subMode pass through as-is (scalar); default if absent
    mode: state.mode ?? DEFAULT_STATE.mode,
    pipeline: state.pipeline ?? DEFAULT_STATE.pipeline,
    subMode: state.subMode ?? DEFAULT_STATE.subMode,
    // NF objects: preserve whatever is on disk; default to empty
    bookDna: state.bookDna ?? {},
    nfStages: state.nfStages ?? {},
    // Fiction-specific (unchanged)
...
export function isNonfiction(state) {
  return state?.mode === 'nonfiction';
}

export async function saveState(state) {
  const stateDir = resolve(process.cwd(), '.storyline');
...
export function printStatus(state) {
  ...
  if (currentStage) {
    ...
    console.log(chalk.bold('\n✅ All stages complete — run `storyline generate` to create master document\n'));
  }
  console.log();
}
