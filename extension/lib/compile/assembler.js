...
function buildMetadata(state, config) {
  ...
  return {
    ...
    tags: override.tags || [],
    // Pass through the cover image path (relative to project root or
    // absolute) so the EPUB packager can embed it. Without this, the
    // packager's `metadata.coverImage` is undefined and the resulting
    // EPUB ships without a cover even when one is configured.
