...
**Status:** PARTIAL (extension-v0.2.29 — see CB-11b for round-trip parsing)

**Outcome:** New `extension/src/state/stage-md-watcher.ts` watches `planning/stages/*.md`. Self-writes (Storyline's own `writeStageDoc` calls) are suppressed via a path+timestamp window. On a real user edit it shows a one-time per-session toast: *"Stage docs are auto-generated by Storyline — manual edits are overwritten on the next save. To update permanently, ask in the planning chat."* New command `storyline.resetStageDoc` regenerates a stage doc from state.json — useful when the user has accidentally clobbered the auto-rendered file.

Round-trip parsing (turning user edits in the .md back into a state-patch) is intentionally deferred to **CB-11b** — it needs a label-to-key map per stage, and lossy/opaque renderer patterns (tables, nested lists) make it brittle. The MVP gives users the truth (their edits won't survive) plus an undo path; the systematic round-trip is its own focused project.
...
**Status:** PARTIAL (extension-v0.2.29 — see CB-13b/c for diff viewer + branch-at-stage) · **Effort:** L (1 week+) · **Risk:** medium

**Outcome:** New `extension/src/manuscript/versions.ts` adds two writer-language commands: `Storyline: Save as Version…` and `Storyline: Versions…`. Implementation rides on the existing isomorphic-git wrapper but the words "branch" and "commit" never reach the UI. Saving a version creates a `version/<slug>` branch from the current state without switching the writer to it; their working draft is preserved. Listing versions shows a quick-pick with a "Back to working draft" entry when on a version branch. Switching auto-commits the writer's WIP first so they don't lose live edits. v1 ships without the prose-aware diff viewer (CB-13b) and without the auto-branch-at-stage flow (CB-13c — "try a different ending" branches at the climax beat).
