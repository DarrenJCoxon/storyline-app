...
### Milestone 6.5 — Compile v2: Beating Vellum on output quality (proposed)
**Outcome:** Storyline's compiled output is visibly more refined than Vellum's. Bundled OFL fonts with real OpenType features (true small caps, optical sizes, old-style figures), twelve hand-tuned Book Styles where every element is paired by design, layout primitives for non-fiction and illustrated work (epigraphs, pull quotes, sidebars, real footnotes, plates, verse, marginalia), generated front and back matter (half-title, copyright, real ToC with dotted leaders, about-the-author, also-by, index), per-store export profiles (Apple / Kindle / Kobo / KDP / IngramSpark) with auto-calculated spine width and full-cover PDFs, and a three-device live preview with Book-Style hot-swap.
**Prove-it gate:** A blind reader picks the Phase-1+2 output over today's Storyline output and over Vellum's output in a side-by-side. A non-fiction project compiled in Ledger looks like a real non-fiction book. A picture book compiled in Quarto handles full-bleed. One Compile produces the full distribution bundle that every store accepts without warnings.
**Status:** Proposed. Six independent phases with their own gates — see the detailed plan: [roadmap/compile-v2/README.md](roadmap/compile-v2/README.md). Smallest shippable unit: Phase 1 (typography foundation) + first three Book Styles in Phase 2.
