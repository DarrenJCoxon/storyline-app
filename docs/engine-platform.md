...
Storyline is the first engine. It's not the whole product.
...
**Storyline** (already built as a CLI + skill; this is the first engine)
...
5. **State management** — the `.storyline/state.json` pattern, but generalised per-engine
...
Critique at stage boundaries is routed to a specific Claude model (Haiku / Sonnet / Opus) via subagent delegation rather than run on whatever model the parent harness session happens to be using. Mapping lives in `lib/ai/model-router.js` and `skill/docs/routing/stage-model-map.md`; the CLI exposes `storyline-vsc route <stageId>` and `storyline-vsc record-model <stageId> <model>` for skill-side orchestration, and `storyline-vsc config set ai.quality economy|balanced|premium` for the writer.
...
1. **A Claude Code skill** — the planning conversation (today's `skill/SKILL.md` for Storyline)
2. **A state schema** — what data gets tracked through planning (today's `lib/state/project-state.js`)
3. **A CLI** — stage management, state persistence, memory writing (today's `bin/storyline-writer.js`)
...
"Storyline" is a specific engine. The platform needs its own name. Options:
...
Nothing here is novel — it's the existing Storyline code generalised to be one implementation of an interface. The refactor is: extract the platform bits out of `storyline`, leave the novel-specific bits in a `storyline-engine` package.

## Refactor path (from current Storyline code)

The current repo mixes platform and engine concerns because until today we thought "Storyline" was the whole product. The split:
...
**Storyline engine (rename current repo):**
...
- `skill/SKILL.md` — the /storyline command
- Novel-specific state shape (protagonist, beats, etc)
- Novel-specific compile themes (Classic Serif etc)

The split is mostly mechanical. Today's `storyline` is about 70% engine-specific, 30% platform. Extracting the platform gives us the substrate; adding new engines becomes roughly as much work as the current Storyline was.
...
## What this means for the current Storyline work

Immediate: **nothing changes.** Don't refactor yet. Finish Storyline as a functional CLI + skill. Use it to plan and write a real book. Learn what the engine needs to expose.
...
The right sequence:
1. Finish Storyline (planning harness + compile pipeline for novels)
...
Nothing to do right now. This doc exists to capture the insight before it evaporates. When the time comes (after Storyline is working end-to-end and the VS Code extension prototype has been built), this doc becomes the guide for the refactor.

The single most important thing this doc says: **one engine working real-world is worth more than three engines designed on paper.** Don't start the platform work until Storyline ships.
