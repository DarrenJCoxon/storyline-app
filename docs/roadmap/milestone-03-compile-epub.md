...
**CLI shelling from VS Code cross-platform.** `spawn('nw', ...)` assumes `storyline` is on PATH. On macOS with npm link, yes. On Windows with fresh install, may not be. Use `npx storyline-vsc compile` as the spawn target to route through Node's module resolution. Verify in Story 3.7.
