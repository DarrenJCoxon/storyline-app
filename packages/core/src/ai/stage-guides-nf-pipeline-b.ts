...
export const PIPELINE_B_GUIDES: Record<string, NfPipelineGuide> = {
  'pb-thesis': {
    id: 'pb-thesis',
  ...
  'pb-chapters': {
    ...
    index: 8,
    persona: 'structural-editor',
    opening: `The chapter outline differs based on your structural fork.\n\n**Idea-led**: each chapter = one big idea, supported by narrative examples. The chapter order follows the argument, not the calendar.\n\n**Event-led**: each chapter = a period or sequence of events. The chapter order is chronological (or causal). Ideas emerge from events rather than driving them.\n\nWe go two levels deep: the chapter (question, role, momentum) and the sections within it (opening scene, narrative scenes, idea statement, evidence, closing). This is the skeleton — not prose.`,
    questions: [
      {
        key: 'chapters',
        label: 'List each chapter with its question, key content, section breakdown, and sourcing notes',
        hint: 'For each chapter: question it answers, anchor scene, key sources, and a section map (opening/hook, 2–4 narrative or evidence scenes, idea/argument statement, close/transition).',
        type: 'array',
        itemSchema: {
          ...
          anchorScene: 'The most important scene or moment in this chapter',
          sourcingNote: 'Key sources, archives, or interviews this chapter relies on',
      ...
      },
      {
        ...
        hint: 'Sum of per-chapter estimates. Should align with target from Book DNA.',
        required: false,
      },
    ],
...
export function getPipelineBGuide(stageId: string): NfPipelineGuide | null {
  return PIPELINE_B_GUIDES[stageId] ?? null;
}

export const PIPELINE_B_GUIDE_ORDER: NfPipelineGuide[] = Object.values(PIPELINE_B_GUIDES)
