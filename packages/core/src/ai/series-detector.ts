...
export function detectSeriesPotential(
  premiseData: Record<string, unknown>,
  genreData: Record<string, unknown>,
): SeriesPotentialResult {
  const indicators: SeriesIndicator[] = []

  if (premiseData?.rawLogline) {
    const logline = (premiseData.rawLogline as string).toLowerCase()
    ...
    if (genreData?.primaryGenre && seriesFriendlyGenres.includes((genreData.primaryGenre as string).toLowerCase())) {
      indicators.push({
        type: 'genre',
        score: 'medium',
        reason: `${genreData.primaryGenre as string} readers often expect series — they want to return to this world.`,
      })
    }

    if (premiseData?.conceptHook && (premiseData.conceptHook as string).length > 200) {
      indicators.push({
        type: 'complexity',
        score: 'low',
