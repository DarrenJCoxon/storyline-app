...
export interface PrerequisiteChainResult {
  markdown: string
  cycles: number[][]
  forwardRefs: Array<{ chapter: number; prereq: number }>
  topologicalOrder: number[]
}

export function generatePrerequisiteChain(plan: AcademicPlan): PrerequisiteChainResult {
  const { chapters } = plan
  const chapterNumbers = chapters.map(c => c.number)
  const prereqMap: Record<number, number[]> = {}
  for (const ch of chapters) prereqMap[ch.number] = ch.prerequisites

  // Forward references: chapter N listing a chapter M where M >= N
  const forwardRefs: Array<{ chapter: number; prereq: number }> = []
  ...
  for (const n of chapterNumbers) inDegree[n] = 0
  for (const ch of chapters) {
    for (const p of ch.prerequisites) {
      if (chapterNumbers.includes(p)) inDegree[ch.number] = (inDegree[ch.number] ?? 0) + 1
    }
  }
  ...
  while (queue.length) {
    ...
    for (const ch of chapters) {
      if (ch.prerequisites.includes(n)) {
        inDegree[ch.number]--
        if (inDegree[ch.number] === 0) queue.push(ch.number)
      }
    }
    queue.sort((a, b) => a - b)
  }
