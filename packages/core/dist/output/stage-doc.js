...
function nfStructuredList(items) {
    if (!Array.isArray(items) || items.length === 0)
        return '';
    const allPrimitive = items.every(i => i == null || ['string', 'number', 'boolean'].includes(typeof i));
    if (allPrimitive) {
...
const nfRenderers = {
    ...
    'dna-comps'(state) {
        ...
        const comps = Array.isArray(s.comps) ? s.comps : [];
        if (comps.length) {
            md += heading('Comparable Titles', 3);
            comps.forEach((c, i) => {
                const title = c.title || `Comp ${i + 1}`;
                const author = c.author ? ` — ${c.author}` : '';
                md += `**${i + 1}. ${title}${author}**\n\n`;
                if (c.year)
                    md += `- **Year:** ${c.year}\n`;
                if (c.whatItDoes)
                    md += `- **What it does:** ${c.whatItDoes}\n`;
                if (c.whatTheyGotRight)
                    md += `- **What they got right:** ${c.whatTheyGotRight}\n`;
                if (c.yourGap)
                    md += `- **Your gap:** ${c.yourGap}\n`;
                else if (c.gap)
                    md += `- **Gap:** ${c.gap}\n`;
                md += '\n';
            });
    ...
    'pa-framework'(state) {
        ...
        if (steps.length) {
            ...
            const allPrimitive = steps.every(i => i == null || ['string', 'number', 'boolean'].includes(typeof i));
            if (allPrimitive)
                md += nfList(steps);
            else
                md += nfStructuredList(steps);
        }
        ...
        if (principles.length) {
            ...
            principles.forEach((p, i) => {
                ...
                const name = p.name ?? (typeof p === 'string' ? p : `Principle ${num}`);
                const def = p.definition ?? p.claim ?? '';
                md += def ? `**${num}. ${name}** — ${def}\n\n` : `**${num}. ${name}**\n\n`;
            });
        }
        return md;
    ...
    'pa-objections'(state) {
        ...
        const objs = Array.isArray(s.objections) ? s.objections : [];
        objs.forEach((o, i) => {
            const obj = o.objection ?? (typeof o === 'string' ? o : `Objection ${i + 1}`);
            md += heading(`${i + 1}. ${obj}`, 3);
            if (o.source)
                md += nfLine('Source', o.source);
            if (o.response)
                md += nfLine('Response', o.response);
    ...
    'pa-principles'(state) {
        ...
        ps.forEach((p, i) => {
            const num = p.number ?? i + 1;
            const name = p.name ?? p.principle ?? `Principle ${num}`;
    ...
    'pa-evidence'(state) {
        ...
        const byPrinciple = Array.isArray(s.evidenceByPrinciple)
            ? s.evidenceByPrinciple
            : [];
        byPrinciple.forEach(p => {
            const num = p.principleNumber ?? '?';
            const label = p.principleLabel ?? '';
            md += heading(`Principle ${num}${label ? ` — ${label}` : ''}`, 3);
            const items = Array.isArray(p.evidenceItems) ? p.evidenceItems : [];
            items.forEach(item => {
                ...
                const supports = it.supportsTheClaim ?? '';
...
function genericRender(stageId, state) {
    const nf = (state.nfStages ?? {});
    const top = state;
    ...
    if (!body.trim())
        return null;
    const stageName = project_state_js_1.STAGE_BY_ID[stageId]?.name ??
        project_state_js_1.NF_STAGE_BY_ID[stageId]?.name ??
        humanLabel(stageId);
