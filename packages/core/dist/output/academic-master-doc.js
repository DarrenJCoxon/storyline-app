...
exports.generateAcademicMasterDocument = generateAcademicMasterDocument;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const learning_outcome_coverage_js_1 = require("./learning-outcome-coverage.js");
const prerequisite_chain_js_1 = require("./prerequisite-chain.js");
...
function generateAcademicMasterDocument(plan, state, projectDir) {
    ...
    const coverage = (0, learning_outcome_coverage_js_1.generateLearningOutcomeCoverage)(academic);
    const prereq = (0, prerequisite_chain_js_1.generatePrerequisiteChain)(academic);
    const glossary = (0, glossary_js_1.generateGlossary)(academic);
    const exIndex = (0, exercise_index_js_1.generateExerciseIndex)(academic);
    ...
    if (corePromise !== '—')
        lines.push(`**Core promise:** ${corePromise}`);
    const dnaReader = stg('dna-reader');
    const targetReader = str(dnaReader.targetReader ?? dnaReader.reader);
    if (targetReader !== '—')
        lines.push(`**Target reader:** ${targetReader}`);
    lines.push('');
    // ── Outcome inventory ───────────────────────────────────────────────────────
    lines.push('## Outcome inventory', '');
    lines.push(`**Total outcomes:** ${academic.learningOutcomes.length}`);
    lines.push(`**Coverage:** ${academic.learningOutcomes.length - coverage.gaps.length} / ${academic.learningOutcomes.length} (${academic.learningOutcomes.length > 0
    ...
    if (coverage.doubleCovered.length)
        lines.push(`**Double-covered:** ${coverage.doubleCovered.join(', ')}`);
    lines.push('');
    ...
    for (const ch of academic.chapters) {
        ...
        if (ch.wordTarget)
            lines.push(`*~${ch.wordTarget.toLocaleString()} words*`);
        if (ch.prerequisites.length)
            lines.push(`*Prerequisites: ${ch.prerequisites.map(n => `${unitLabel} ${n}`).join(', ')}*`);
        if (ch.outcomes.length)
            lines.push(`*Outcomes: ${ch.outcomes.join(', ')}*`);
        if (ch.keyTerms.length)
            lines.push(`*Key terms: ${ch.keyTerms.join(', ')}*`);
        if (academic.bookType === 'textbook') {
            if (ch.workedExamples.length)
                lines.push(`*Worked examples: ${ch.workedExamples.map(w => w.id).join(', ')}*`);
            if (ch.exercises.length)
                lines.push(`*Exercises: ${ch.exercises.map(e => e.id).join(', ')}*`);
        }
        else {
            if (ch.recallQuestions)
                lines.push(`*Recall questions: ${ch.recallQuestions}*`);
            if (ch.examPractice?.length)
                lines.push(`*Exam practice: ${ch.examPractice.map(ep => `${ep.count} ${ep.type}`).join(', ')}*`);
        }
        lines.push('');
    }
...
//# sourceMappingURL=academic-master-doc.js.map