...
exports.runFullCritique = runFullCritique;
exports.buildSummaryMarkdown = buildSummaryMarkdown;
exports.generateCritiqueReport = generateCritiqueReport;
...
function checkDnaToPipelineCoherence(state) {
    ...
    if (pipeline === 'A') {
        ...
        if (principles.length > 0 && chapters.length > 0) {
            if (principles.length > bodyChapters.length + 2) {
                findings.push(finding('pa-principles-unlinked', 'warning', 'coherence', 'dna-pipeline-coherence', 'pa-framework → pa-chapters', `${principles.length} framework principles but only ${bodyChapters.length} chapter(s) linked to a principle. ${principles.length - bodyChapters.length} principle(s) may have no chapter home.`, 'Add linkedPrinciple to all body chapters or consolidate principles.'));
            }
        }
        ...
        if (paFramework.modelName && paThesis.thesis) {
            ...
            if (!anyMatch && modelWords.length > 0) {
                findings.push(finding('pa-thesis-framework-drift', 'warning', 'drift', 'dna-pipeline-coherence', 'pa-thesis → pa-framework', `The thesis and framework model name ("${paFramework.modelName}") share no common language. They may have drifted apart during planning.`, 'Ensure the framework is the answer to the thesis — the names should share conceptual vocabulary.'));
            }
        }
        ...
        if (dnaEvidence.evidenceTypes && /anecdote/i.test(dnaEvidence.evidenceTypes)
            ...
            if (!paEvidence.strongestEvidence || /anecdote|story|personal/i.test(paEvidence.strongestEvidence)) {
                findings.push(finding('pa-anecdote-only-evidence', 'warning', 'coherence', 'dna-pipeline-coherence', 'dna-evidence → pa-evidence', 'Evidence philosophy relies on anecdote only, and the Evidence Map reflects this. Prescriptive books built on anecdote alone face credibility challenges.', 'Identify at least one statistic, case study, or peer-reviewed finding per principle.'));
            }
        }
        ...
        if (dnaReader.biggestFear) {
            ...
            if (openerText && !anyMatch) {
                findings.push(finding('pa-opener-fear-mismatch', 'tip', 'drift', 'dna-pipeline-coherence', 'dna-reader → pa-opener', 'The opening scene may not address the reader\'s biggest fear from Book DNA Stage 2. The opener should make that fear viscerally real.', `Reader's biggest fear: "${dnaReader.biggestFear}". Check that the opener scene resonates with it.`));
            }
        }
    }
    ...
    if (pipeline === 'B') {
        ...
        if (pbThesis.centralQuestion && chapters.length > 0) {
            ...
            if (closingText && !anyMatch && !pbCritique.centralQuestionAnsweredCheck) {
                findings.push(finding('pb-closing-doesnt-answer-question', 'warning', 'promise-payoff', 'dna-pipeline-coherence', 'pb-thesis → pb-chapters (final)', 'The last chapter may not answer the central question from Stage 1. Narrative non-fiction that doesn\'t answer its own question fails structurally.', `Central question: "${pbThesis.centralQuestion}". Verify the closing chapter explicitly answers it.`));
            }
        }
        ...
        if (pbTimeline.timelineSpan && chapters.length > 0 && chapters.length < 5) {
            findings.push(finding('pb-few-chapters-for-span', 'tip', 'coherence', 'dna-pipeline-coherence', 'pb-timeline → pb-chapters', `Timeline spans "${pbTimeline.timelineSpan}" but only ${chapters.length} chapter(s) are planned. This ratio may compress important events into thin coverage.`, 'Consider whether the timeline scope and chapter count are calibrated correctly.'));
        }
    }
    ...
    if (pipeline === 'C') {
        ...
        if (pcEndState.measurableOutcome && !pcMilestones.finalAssessment) {
            findings.push(finding('pc-end-state-not-assessed', 'error', 'promise-payoff', 'dna-pipeline-coherence', 'pc-end-state → pc-milestones', `End-state competency is defined ("${pcEndState.measurableOutcome.slice(0, 60)}…") but there is no final assessment milestone. The book cannot verify its own outcome.`, 'Design a final assessment in Stage 8 that directly tests the measurable outcome.'));
        }
        ...
        if (pcStartLevel.startingLevel && drills.length > 0) {
            ...
            if (isAbsBeginner && !hasBeginnerDrill) {
                findings.push(finding('pc-drill-level-mismatch', 'warning', 'drift', 'dna-pipeline-coherence', 'pc-start-level → pc-drills', 'Reader is defined as an absolute beginner, but no drills are marked as "beginner" difficulty.', 'Mark entry-level drills as difficulty: "beginner" to match the reader starting level.'));
            }
        }
    }
    ...
    if (dnaPromise.corePromise && dnaTransform.transformationSentence) {
        ...
        if (!anyMatch) {
            findings.push(finding('promise-transform-drift', 'warning', 'coherence', 'dna-pipeline-coherence', 'dna-promise → dna-transform', 'The Core Promise (Stage 6) and the Transformation Sentence (Stage 3) share no common language. They should reinforce the same reader journey.', 'Align the transformation sentence with the language of the core promise.'));
        }
    }
    ...
    if (dnaIdea.bigIdea && pipeline === 'A') {
        ...
        if (thesisText && !ideaWords.some(w => thesisText.includes(w))) {
            findings.push(finding('pa-big-idea-thesis-drift', 'tip', 'drift', 'dna-pipeline-coherence', 'dna-idea → pa-thesis', 'The Big Idea from Book DNA Stage 4 and the Pipeline A thesis share no common language. The thesis should be a sharpened expression of the Big Idea.', `Big Idea: "${dnaIdea.bigIdea.slice(0, 80)}". Ensure the thesis refines rather than departs from it.`));
        }
    }
    return findings;
}
...
function checkReaderAvatarDrift(state) {
    ...
    const pipeline = state?.pipeline;
    const dnaReader = nf['dna-reader'] || {};
    if (!dnaReader.avatarName)
        return findings; // No avatar defined — nothing to drift from
    ...
    if (pipeline === 'A') {
        ...
        if (dnaReader.deepestWish && paOpener.closerVision) {
            ...
            if (!wishWords.some(w => closerText.includes(w))) {
                findings.push(finding('pa-closer-avatar-drift', 'warning', 'drift', 'reader-avatar-drift', `dna-reader → pa-opener (closer)`, `Closer vision may not speak to ${avatarName}'s deepest wish. The closer should deliver what the reader came for.`, `Deepest wish: "${dnaReader.deepestWish}". Review the closer vision against it.`));
            }
        }
        ...
        if (dnaReader.demographics && apps.length > 0) {
            ...
            if (isBusy && hasLongAction) {
                findings.push(finding('pa-actions-dont-fit-reader', 'tip', 'drift', 'reader-avatar-drift', 'dna-reader → pa-application', `${avatarName} is described as busy/time-poor, but some application actions require months or major overhauls. Actions should fit the reader's constraints.`, 'Trim or reframe long-horizon actions for the defined reader profile.'));
            }
        }
    }
    ...
    if (pipeline === 'B') {
        ...
        if (pbFork.subMode === 'idea-led' && cast.length > 0) {
            ...
            if (!hasRelatable && cast.length <= 3) {
                findings.push(finding('pb-cast-no-reader-anchor', 'tip', 'drift', 'reader-avatar-drift', 'dna-reader → pb-cast', 'In idea-led narrative NF, the reader needs a human anchor in the cast — someone whose experience they can follow. Consider whether the cast has such a figure.', 'Review cast roles. A "witness" or reader-surrogate character aids the idea-led structure.'));
            }
        }
    }
    ...
    if (pipeline === 'C') {
        ...
        if (pcStartLevel.assumedKnowledge && /none|nothing|no knowledge|zero/i.test(pcStartLevel.assumedKnowledge)) {
            const technicalLessons = lessons.filter(l => l.keyConceptsCount && parseInt(l.keyConceptsCount) > 3);
            if (technicalLessons.length > lessons.length / 2) {
                findings.push(finding('pc-lessons-too-dense-for-beginner', 'warning', 'drift', 'reader-avatar-drift', 'pc-start-level → pc-lessons', `Reader starts with zero assumed knowledge, but ${technicalLessons.length} of ${lessons.length} lesson(s) introduce more than 3 concepts. Concept-dense lessons lose beginners.`, 'Limit beginner lessons to 1–2 core concepts. Split dense lessons across chapters.'));
            }
        }
    }
    return findings;
...
function checkPromisePayoff(state) {
    ...
    if (pipeline === 'A') {
        ...
        if (chapters.length > 0) {
            ...
            if (!delivered) {
                findings.push(finding('pa-promise-undelivered', 'warning', 'promise-payoff', 'promise-payoff-audit', 'dna-promise → pa-chapters', 'The Core Promise doesn\'t map to any chapter title or job description. Readers who bought the promise will not find where it\'s kept.', `Core promise: "${dnaPromise.corePromise.slice(0, 80)}". Map it explicitly to a chapter job.`));
            }
        }
        ...
        if (dnaPromise.subtitleDraft) {
            ...
            if (!delivered && planText.length > 20) {
                findings.push(finding('pa-subtitle-not-reflected', 'tip', 'promise-payoff', 'promise-payoff-audit', 'dna-promise → pa-thesis / pa-framework', `The subtitle ("${dnaPromise.subtitleDraft.slice(0, 60)}") isn\'t reflected in the thesis or framework name. The subtitle is the most commercially read sentence — the plan must deliver it.`, 'Align thesis and/or framework language with the subtitle promise.'));
            }
        }
    }
    ...
    if (pipeline === 'B') {
        ...
        if (chapters.length > 0) {
            ...
            if (closingText.length > 20 && !delivered) {
                findings.push(finding('pb-closing-doesnt-deliver-promise', 'warning', 'promise-payoff', 'promise-payoff-audit', 'dna-promise → pb-chapters (closing)', 'The Core Promise isn\'t reflected in the closing chapter. The final chapter must deliver what the promise implied.', `Core promise: "${dnaPromise.corePromise.slice(0, 80)}".`));
            }
        }
    }
    ...
    if (pipeline === 'C') {
        ...
        if (pcEndState.measurableOutcome) {
            ...
            if (!delivered) {
                findings.push(finding('pc-promise-outcome-drift', 'warning', 'promise-payoff', 'promise-payoff-audit', 'dna-promise → pc-end-state', 'The Core Promise and the End-State Competency (Stage 3) share no common language. The measurable outcome must be the concrete form of the promise.', `Promise: "${dnaPromise.corePromise.slice(0, 60)}". Outcome: "${pcEndState.measurableOutcome.slice(0, 60)}".`));
            }
        }
    }
    return findings;
...
function checkCompAdjacency(state) {
        ...
        if (paFramework.modelName) {
            ...
            if (genericPatterns.test(paFramework.modelName)) {
                ...
                if (compTitles.includes('laws') || compTitles.includes('habits') || compTitles.includes('rules')) {
                    findings.push(finding('pa-framework-generic-name', 'warning', 'comps', 'comp-adjacency', 'dna-comps → pa-framework', `Framework name "${paFramework.modelName}" follows the same naming pattern as one or more comps. This reduces differentiation.`, 'Rename the framework to something more conceptually specific — the model name is a brand asset.'));
                }
            }
        }
    }
    ...
    if (dnaIdea.whyDifferent) {
        if (/(better|more|less|new|fresh|unique|different)/i.test(dnaIdea.whyDifferent)
            ...
            findings.push(finding('differentiation-vague', 'warning', 'comps', 'comp-adjacency', 'dna-idea (whyDifferent)', 'Differentiation from comps relies on comparative language ("better", "new", "unique") without specifics. This won\'t survive a pitch meeting or an Amazon reader comparing two books.', 'Name exactly what is different — the specific mechanism, approach, or evidence that comps lack.'));
        }
    }
    else if (comps.length >= 3) {
        findings.push(finding('differentiation-undefined', 'error', 'comps', 'comp-adjacency', 'dna-idea → dna-comps', `${comps.length} comps identified but differentiation ("whyDifferent") is not defined. Without it, the book occupies the same market space as the comps.`, 'Complete dna-idea with a specific differentiation statement.'));
    }
    // Check comps without gap analysis
    const noGap = comps.filter(c => !c.yourGap || c.yourGap.length < 20);
    if (noGap.length > 1) {
        findings.push(finding('comps-no-gap-analysis', 'warning', 'comps', 'comp-adjacency', 'dna-comps', `${noGap.length} comp(s) have no gap analysis. The gap is why your book exists — without naming it per comp, adjacency risk is unquantified.`, 'Add specific gap analysis for each comp: what does it not do that your book does?'));
    }
    return findings;
}
...
function integrateResearchFindings(researchGaps) {
    ...
    if (unsourcedItems.length > 5) {
        findings.push(finding('research-many-unsourced', 'warning', 'research', 'research-gap-integration', 'research subsystem', `${unsourcedItems.length} research items are pending with no sources attached. Unsourced items are a publication liability.`, 'Prioritise sourcing these items before drafting the chapters they support.'));
    }
    if (thinChapters.length > 0) {
        const chapterList = thinChapters.map(c => `Ch${c.chapterNumber}`).join(', ');
        findings.push(finding('research-thin-chapters', 'warning', 'research', 'research-gap-integration', `chapters: ${chapterList}`, `${thinChapters.length} chapter(s) have fewer than 2 linked research items: ${chapterList}.`, 'Add research items linked to these chapters or accept they will be under-evidenced.'));
    }
    if (lowReliabilityOnly.length > 0) {
        const chapterList = lowReliabilityOnly.map(c => `Ch${c.chapterNumber}`).join(', ');
        findings.push(finding('research-anecdote-only-chapters', 'warning', 'research', 'research-gap-integration', `chapters: ${chapterList}`, `${lowReliabilityOnly.length} chapter(s) have only anecdotal evidence: ${chapterList}.`, 'Add at least one primary, peer-reviewed, or secondary source to each of these chapters.'));
    }
    if (stats.total > 0 && stats.primaryOrPeerReviewed / stats.total < 0.2) {
        findings.push(finding('research-low-primary-ratio', 'tip', 'research', 'research-gap-integration', 'research subsystem', `Only ${stats.primaryOrPeerReviewed} of ${stats.total} research items (${Math.round(stats.primaryOrPeerReviewed / stats.total * 100)}%) are primary or peer-reviewed. Commercial non-fiction typically benefits from a higher ratio.`, 'Identify key claims that need primary or peer-reviewed backing and add them to the research subsystem.'));
    }
    return findings;
}
...
function wrapInStageCritique(state) {
    ...
    let critiqueData = {};
    if (pipeline === 'A')
        critiqueStageId = 'pa-critique';
    if (pipeline === 'B')
        critiqueStageId = 'pb-critique';
    if (pipeline === 'C')
        critiqueStageId = 'pc-critique';
    if (critiqueStageId) {
        critiqueData = nf[critiqueStageId] || {};
        // Flag the biggest structural risk as a named finding
        if (critiqueData.critiqueSummary) {
            findings.push(finding(`${critiqueStageId}-summary`, 'warning', 'in-stage', 'in-stage-critique', critiqueStageId, `Writer-identified risk: "${critiqueData.critiqueSummary}"`, 'This was flagged in the per-stage critique. Ensure it is addressed before drafting.'));
        }
    }
    return findings;
}
// ── Orchestrator ──────────────────────────────────────────────────────────────
function runFullCritique(state, researchGaps = null) {
    const all = [
...
function buildReportMarkdown(critiqueResult, state) {
    ...
    sections.push(`---`, ``, `*This report is generated automatically at \`nf generate\` time.*`, `*Per-stage critique (in-stage errors) is available via \`nf stage-info <stageId>\`.*`);
    return sections.join('\n');
}
function buildSummaryMarkdown(critiqueResult) {
...
async function generateCritiqueReport(state, projectDir = process.cwd(), researchGaps = null) {
    ...
    const outputDir = (0, path_1.resolve)(projectDir, 'output');
    await (0, promises_1.mkdir)(outputDir, { recursive: true });
    const reportPath = (0, path_1.resolve)(outputDir, 'critique-report.md');
    await (0, promises_1.writeFile)(reportPath, reportMd, 'utf-8');
    return {
        reportPath,
...
//# sourceMappingURL=critique-api.js.map