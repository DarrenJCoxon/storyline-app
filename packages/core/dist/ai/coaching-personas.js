...
exports.getPersonaForStage = getPersonaForStage;
exports.runQualityChecklist = runQualityChecklist;
exports.formatPersonaIntro = formatPersonaIntro;
exports.PERSONAS = {
    strategist: {
        name: 'The Strategist',
        stage: 'genre',
...
function getPersonaForStage(stageId) {
    const key = STAGE_PERSONA_MAP[stageId];
    return key ? (exports.PERSONAS[key] ?? null) : null;
}
function runQualityChecklist(stageId, state) {
...
function checkQualityItem(stageId, checkDescription, state) {
    switch (stageId) {
        case 'genre':
            ...
            if (checkDescription.includes('Audience'))
        ...
        case 'protagonist': {
            ...
            if (checkDescription.includes('external and specific'))
                return !!p?.want;
            if (checkDescription.includes('internal and emotional'))
                return !!p?.need;
            if (checkDescription.includes('DIFFERENT'))
                return !!(p?.want && p?.need && p.want.toLowerCase() !== p.need.toLowerCase());
            if (checkDescription.includes('Ghost drives'))
                return !!p?.ghost;
            if (checkDescription.includes('sabotages'))
                return !!p?.flaw;
            if (checkDescription.includes('Core lie'))
                return !!p?.coreLie;
            if (checkDescription.includes('Arc direction'))
                return !!p?.arcDirection;
            return true;
        }
        case 'beatSheet': {
            const beats = state.beatSheet?.beats ?? {};
            if (checkDescription.includes('choice, not a push'))
                return !!beats.beat05BreakIntoTwo?.threshold;
            if (checkDescription.includes('reversal'))
                return !!beats.beat08Midpoint?.midpointType;
            if (checkDescription.includes('whiff of death'))
                return !!beats.beat10AllIsLost?.whiffOfDeath;
            if (checkDescription.includes('earned'))
                return !!beats.beat12Beat13?.secondDoorway;
            if (checkDescription.includes('transformation'))
                return !!beats.beat13Finale?.selfRevelation;
...
function formatPersonaIntro(stageId) {
    const persona = getPersonaForStage(stageId);
    if (!persona)
        return '';
    return `${persona.name} — ${persona.tagline}\n\n${persona.activation}`;
}
//# sourceMappingURL=coaching-personas.js.map