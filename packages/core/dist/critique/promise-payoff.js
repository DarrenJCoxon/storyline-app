...
exports.checkNfPromisePayoff = checkNfPromisePayoff;
exports.findFictionPromiseGaps = findFictionPromiseGaps;
 ...
 */
function checkNfPromisePayoff(plan) {
    const findings = [];
    if (!plan.nfPromise?.corePromise)
        return findings;
    ...
    if (pipeline === 'A') {
        ...
        if (subtitleDraft) {
            const subtitleWords = subtitleDraft.toLowerCase().split(/\s+/).filter((w) => w.length > 4).slice(0, 4);
            ...
            if (!delivered && planText.length > 20) {
                findings.push(finding('pa-subtitle-not-reflected', 'tip', 'promise-payoff', 'promise-payoff-audit', 'dna-promise → pa-thesis / pa-framework', `The subtitle ("${subtitleDraft.slice(0, 60)}") isn't reflected in the thesis or framework name. The subtitle is the most commercially read sentence — the plan must deliver it.`, 'Align thesis and/or framework language with the subtitle promise.'));
            }
        }
    }
    ...
    if (pipeline === 'B') {
        if (plan.nfChapters.length > 0) {
            ...
            if (closingText.length > 20 && !delivered) {
                findings.push(finding('pb-closing-doesnt-deliver-promise', 'warning', 'promise-payoff', 'promise-payoff-audit', 'dna-promise → pb-chapters (closing)', "The Core Promise isn't reflected in the closing chapter. The final chapter must deliver what the promise implied.", `Core promise: "${corePromise.slice(0, 80)}".`));
            }
        }
    }
    // Pipeline C: promise must match end-state competency
    if (pipeline === 'C' && endStateMeasurableOutcome) {
        const outcomeWords = endStateMeasurableOutcome.toLowerCase().split(/\s+/).filter((w) => w.length > 4).slice(0, 4);
        const delivered = promiseKeywords.some((w) => endStateMeasurableOutcome.toLowerCase().includes(w))
            || outcomeWords.some((w) => corePromise.toLowerCase().includes(w));
        if (!delivered) {
            findings.push(finding('pc-promise-outcome-drift', 'warning', 'promise-payoff', 'promise-payoff-audit', 'dna-promise → pc-end-state', 'The Core Promise and the End-State Competency (Stage 3) share no common language. The measurable outcome must be the concrete form of the promise.', `Promise: "${corePromise.slice(0, 60)}". Outcome: "${endStateMeasurableOutcome.slice(0, 60)}".`));
        }
    }
    return findings;
}
 ...
 */
function findFictionPromiseGaps(plan) {
    const gaps = [];
    for (const p of plan.promises) {
        if (p.status === 'paid-off')
            continue;
...
//# sourceMappingURL=promise-payoff.js.map