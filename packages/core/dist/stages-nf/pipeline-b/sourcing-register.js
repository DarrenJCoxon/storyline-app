...
exports.buildSourcingRegister = buildSourcingRegister;
const fs_extra_1 = __importDefault(require("fs-extra"));
const { ensureDir, writeFile } = fs_extra_1.default;
const path_1 = require("path");
const capture_js_1 = require("../../research/capture.js");
const SOURCING_DIR = (projectDir) => (0, path_1.join)(projectDir, '.storyline', 'sourcing');
...
function formatRegisterMarkdown(items, groups) {
    ...
    if (items.length === 0) {
        lines.push(`*No sourced claims yet. Add sources via:*`, `\`\`\``, `npx storyline-vsc research add --subtype sourced-claim --link scene:ch1-s1`, `\`\`\``, '');
        return lines.join('\n');
    }
...
async function buildSourcingRegister(projectDir) {
    const dir = SOURCING_DIR(projectDir);
    await ensureDir(dir);
    const allItems = await (0, capture_js_1.listItems)(projectDir);
    ...
    const register = {
        ...
        byLink: Object.fromEntries(Object.entries(groups).map(([k, v]) => [k, v.map(i => i.id)])),
    };
    const jsonPath = (0, path_1.join)(dir, 'register.json');
    const mdPath = (0, path_1.join)(dir, 'register.md');
...
//# sourceMappingURL=sourcing-register.js.map