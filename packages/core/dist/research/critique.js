...
exports.formatGapsReport = formatGapsReport;
...
// ── Gap analysis ──────────────────────────────────────────────────────
async function analyzeGaps(projectDir, state) {
    ...
    for (const ch of chapters) {
        const linked = await (0, linker_js_1.getItemsForChapter)(projectDir, ch.chapterNumber);
    ...
    const unsourcedItems = items.filter(item => item.verification === 'pending' &&
        (!item.sources || item.sources.length === 0)).map(item => ({ id: item.id, title: item.title, subtype: item.subtype }));
    ...
    const stats = {
        ...
        primaryOrPeerReviewed: items.filter(i => i.reliability === 'primary' || i.reliability === 'peer-reviewed').length,
    };
    return {
...
// ── CLI display ───────────────────────────────────────────────────────
function formatGapsReport(findings) {
    ...
    if (unsourcedItems.length) {
        ...
        if (unsourcedItems.length > 10)
            lines.push(`  … and ${unsourcedItems.length - 10} more`);
    }
