...
exports.buildResearchMemoryEntries = buildResearchMemoryEntries;
exports.syncResearchToMemory = syncResearchToMemory;
const fs_extra_1 = __importDefault(require("fs-extra"));
const { ensureDir, writeFile, readFile, pathExists } = fs_extra_1.default;
const path_1 = require("path");
const capture_js_1 = require("./capture.js");
const stage_memory_js_1 = require("../memory/stage-memory.js");
const RESEARCH_DIR = (projectDir) => (0, path_1.join)(projectDir, '.storyline', 'research');
const INDEX_JSON = (projectDir) => (0, path_1.join)(RESEARCH_DIR(projectDir), 'index.json');
const INDEX_MD = (projectDir) => (0, path_1.join)(RESEARCH_DIR(projectDir), 'index.md');
const slugify = (s) => (s || '').toString().toLowerCase()
    .replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 40);
// ── Index rebuild ─────────────────────────────────────────────────────
async function rebuildIndex(projectDir) {
...
async function loadIndex(projectDir) {
    ...
    if (!items.length) {
        ...
        for (const item of items) {
            ...
            if (item.links.length)
                lines.push(`**Linked to:** ${item.links.join(', ')}`);
            if (item.contentPreview)
                lines.push(``, item.contentPreview + (item.contentPreview.length === 200 ? '…' : ''));
            lines.push('');
        }
    }
    return lines.join('\n');
...
function buildResearchMemoryEntries(items, state) {
    const projectSlug = slugify(state?._meta?.projectTitle || state?._meta?.projectPath || 'project');
