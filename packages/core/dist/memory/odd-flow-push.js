...
exports.pushEntriesToOddFlow = pushEntriesToOddFlow;
...
function locateOddFlowCli() {
    ...
    const dep = (0, path_1.resolve)(PROJECT_ROOT, 'node_modules', 'odd-flow', 'bin', 'cli.js');
    if ((0, fs_1.existsSync)(dep))
        return { kind: 'local', path: dep };
    // 2. Dependency of the writer's project (when invoked via npx in their project)
    const projectLocal = (0, path_1.resolve)(process.cwd(), 'node_modules', 'odd-flow', 'bin', 'cli.js');
    if ((0, fs_1.existsSync)(projectLocal))
        return { kind: 'local', path: projectLocal };
    // 3. Fallback: npx will fetch odd-flow on demand.
    return { kind: 'npx' };
}
function buildArgs(cli, subArgs) {
    if (cli.kind === 'local')
        return { cmd: process.execPath, args: [cli.path, ...subArgs] };
...
async function pushOne(entry, cli, cwd) {
    ...
    if (!r.ok && DB_NOT_INIT_RX.test(r.stderr + r.stdout)) {
        ...
        if (initOk)
            r = await runOddFlow(cli, args, { cwd });
    }
...
async function pushEntriesToOddFlow(entries, { cwd = process.cwd() } = {}) {
    ...
    if (pushedIds.length) {
        try {
            await (0, sync_js_1.markSynced)(pushedIds, cwd);
        }
...
//# sourceMappingURL=odd-flow-push.js.map