...
exports.buildNfMemoryEntries = buildNfMemoryEntries;
exports.appendNfMemoryLog = appendNfMemoryLog;
...
// ── Public API ───────────────────────────────────────────────────────────────
function buildNfMemoryEntries(stageId, state) {
...
async function appendNfMemoryLog(entries, projectDir = process.cwd()) {
    if (!entries || entries.length === 0)
        return { entriesWithIds: [], logPath: null };
    ...
    const storyDir = path_1.default.join(projectDir, '.storyline');
    await (0, promises_1.mkdir)(storyDir, { recursive: true });
    const logPath = path_1.default.join(storyDir, 'memory.jsonl');
    const lines = entriesWithIds.map(e => JSON.stringify(e)).join('\n') + '\n';
    await (0, promises_1.appendFile)(logPath, lines, 'utf-8');
    return { entriesWithIds, logPath };
}
//# sourceMappingURL=nf-stage-memory.js.map