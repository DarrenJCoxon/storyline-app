...
exports.buildMemoryEntries = buildMemoryEntries;
exports.appendMemoryLog = appendMemoryLog;
const fs_extra_1 = __importDefault(require("fs-extra"));
const { appendFile, ensureDir } = fs_extra_1.default;
...
const builders = {
    ...
    beatSheet(state, ns, tags) {
        ...
        if (b.overallNotes)
            out.push(entry(ns, 'beats:overall-notes', b.overallNotes, tags));
    ...
    chapterOutline(state, ns, tags) {
        ...
        chapters.forEach(ch => {
            ...
            for (const field of CHAPTER_META_FIELDS) {
                if (ch[field] === undefined || ch[field] === null)
                    continue;
    ...
    critique(state, ns, tags) {
        ...
        (c.flaggedIssues || []).forEach((issue, i) => {
            ...
            out.push(entry(ns, `critique:flagged:${slug}`, `[${issue.severity || 'note'}] ${issue.check ? issue.check + ': ' : ''}${issue.message || ''}${issue.resolution ? ` (resolution: ${issue.resolution})` : ''}`, [...tags, 'critique-issue', issue.severity || 'note']));
        });
        (c.resolvedIssues || []).forEach((issue, i) => {
            const slug = slugify(issue.check || issue.message || `resolved-${i}`) || `resolved-${i}`;
            out.push(entry(ns, `critique:resolved:${slug}`, `${issue.check ? issue.check + ': ' : ''}${issue.message || ''}${issue.resolution ? ` → ${issue.resolution}` : ''}`, [...tags, 'critique-resolved']));
        });
        return out;
    },
...
function buildMemoryEntries(stageId, state) {
    const projectSlug = slugify(state._meta?.projectTitle || (0, path_1.dirname)(state._meta?.projectPath || '') || 'novel');
...
//
// CB-02: projectDir is now explicit. Defaulting to process.cwd() is fine
// for the CLI (bin/storyline.js, which IS launched from the project) but
// dangerous in the VS Code extension host where cwd is wherever VS Code
// itself was launched from. Callers in the extension host (research
// sync, NF stage saves) MUST pass projectDir.
async function appendMemoryLog(entries, projectDir = process.cwd()) {
    if (!entries.length)
        return { logPath: null, entriesWithIds: [] };
    const logDir = (0, path_1.resolve)(projectDir, '.storyline');
    await ensureDir(logDir);
    const logPath = (0, path_1.resolve)(logDir, 'memory.jsonl');
...
//# sourceMappingURL=stage-memory.js.map