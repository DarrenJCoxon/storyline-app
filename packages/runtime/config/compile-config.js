...
//   2. .storyline/state.json metadata (projectTitle, genre, etc.)
//   3. `git config user.name` / `user.email` for author
//   4. Directory basename as last-resort title
//
// Called by `storyline init` (for new projects) and by the compile pipeline
...
export async function buildDefaultConfig(projectPath) {
  ...
  return {
    ...
    pdf: {
      // Print PDF trim size. Supported: '6x9' (trade), '7x10' (textbook),
      // '8x10' (picture book portrait), '8.5x8.5' (picture book square).
      trim: '6x9',
    },
    // chapterOpener (string, optional) — Story 6.5.
    // Controls heading typography (H1, H2) and first-paragraph treatment
    // book-wide. Available openers: meridian, cinder, edgewood, hawthorn.
    // Defaults to the theme's `defaultOpener` value from its theme.json
    // when omitted. Set explicitly to override the theme default.
    // Example: "chapterOpener": "meridian"
  };
}
