// Per-stage markdown renderer — writes planning/stages/<stageId>.md on every save
...
const renderers = {
  ...
  beatSheet(state) {
    ...
    BEAT_ORDER.forEach(bo => {
      ...
      // Beat-specific fields captured by the guide
...
// projectDir defaults to process.cwd() for legacy CLI callers, but VS Code
// callers MUST pass the workspace root — `process.cwd()` in the extension
// host is wherever VS Code was launched from, never the user's project.
// Without the explicit projectDir the file landed outside the project and
// the workspace's planning/stages/<id>.md stayed empty even though saves
// succeeded.
export async function writeStageDoc(stageId, state, projectDir = process.cwd()) {
  const renderer = renderers[stageId];
  if (!renderer) return null;

  const outputDir = join(projectDir, 'planning', 'stages');
  ...
  const filePath = join(outputDir, `${stageId}.md`);
