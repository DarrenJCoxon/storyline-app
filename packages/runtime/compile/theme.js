...
const PRINT_RESET_PATH = resolve(dirname(fileURLToPath(import.meta.url)), 'print-reset.css');
const DEFAULT_THEME_ID = 'classic-serif';
const DEFAULT_PARAGRAPH_STYLE = 'indented';

// Cached so we don't re-read the small reset stylesheet on every
// compile within the same process.
let cachedPrintResetCss = null;
async function loadPrintResetCss() {
  if (cachedPrintResetCss !== null) return cachedPrintResetCss;
  if (await pathExists(PRINT_RESET_PATH)) {
    cachedPrintResetCss = await readFile(PRINT_RESET_PATH, 'utf-8');
  } else {
    cachedPrintResetCss = '';
  }
...
export async function applyTheme(context) {
  ...
  // Print typography reset — cancels body's `text-align: justify` for
  // headings, list items, callouts, etc. so Chromium / Paged.js doesn't
  // stretch single-line elements across the full measure. Loaded BEFORE
  // opener CSS so opener-defined centred chapter titles still win.
  // EPUB readers don't need this (their renderers handle single-line
  // justify correctly).
  const printResetCss = context.format === 'print-pdf' ? await loadPrintResetCss() : '';

  const effectiveCss = theme.css + printResetCss + overrideCss + paragraphStyleOverride(paragraphStyle) + openerCss;
