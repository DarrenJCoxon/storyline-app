...
// Paged.js polyfill — vendored into lib/compile/vendor/ so it survives
// `vsce package --no-dependencies` (CI strips node_modules from the VSIX
// even when allowlisted in .vscodeignore, leaving runtime reads of
// node_modules/pagedjs/ to fail with ENOENT). The vendored copy is the
// source of truth; we fall back to the live node_modules path in dev so
// a fresh pagedjs install is picked up without re-vendoring.
const __pagedHere = dirname(fileURLToPath(import.meta.url));
const PAGED_POLYFILL_CANDIDATES = [
  resolve(__pagedHere, 'vendor', 'paged.polyfill.min.js'),
  resolve(__pagedHere, '..', '..', 'node_modules', 'pagedjs', 'dist', 'paged.polyfill.min.js'),
];
...
async function readPagedPolyfill() {
  const tried = [];
  for (const p of PAGED_POLYFILL_CANDIDATES) {
    try {
      return await readFile(p, 'utf-8');
    } catch (err) {
      tried.push(`${p} (${err.code ?? err.message})`);
    }
  }
  throw new Error(
    `Paged.js polyfill not found. Tried:\n  ${tried.join('\n  ')}\n` +
    `Re-run \`npm run sync-lib\` from the project root, or reinstall the extension.`,
  );
}
...
async function buildPagedHtml({ metadata, theme, trimCss, fontIds = [], bodyClasses = '' }) {
  const pagedPolyfill = await readPagedPolyfill();
  const fontFaceRules = fontIds.length ? printFontFaceBlock(fontIds) + '\n' : '';

  // sectionClass is set by book-style.js (chapters get 'chapter' or 'chapter first-chapter').
  const chaptersHtml = theme.chapters.map(item => renderSection(item)).join('\n');
  const frontHtml    = theme.frontMatter.map(item => renderSection(item)).join('\n');
  const backHtml     = theme.backMatter.map(item => renderSection(item)).join('\n');
...
function renderSection(item) {
  return `  <section class="${escAttr(item.sectionClass || 'section')}" id="${escAttr(item.id)}">
